...
class AuthorServiceImplTest {
    ...
    void shouldSaveAuthor() {
        Author testAuthor = new Author(EXPECTED_ID, EXPECTED_NAME);
    ...
    void shouldUpdateAuthor() {
        ...
        when(authorRepository.findById(any())).thenReturn(Optional.of(testAuthor));
        when(authorRepository.save(any())).thenReturn(testAuthor);
        Author result = authorService.updateAuthor(EXPECTED_ID, testAuthor);

        assertThat(result).isEqualToComparingFieldByField(testAuthor);
    }

    @Test
    @DisplayName("не обновляет автора в БД")
    void shouldNotUpdateAuthor() {
        Author testAuthor = new Author(EXPECTED_NAME);
        when(authorRepository.save(any())).thenReturn(testAuthor);

        assertThrows(ItemNotFoundException.class, () -> authorService.updateAuthor(10L, testAuthor));
    }
